  ...
  return (
    ...
    >
      <div className="pointer-events-none invisible absolute" aria-hidden>
        ...
        <div
          ref={openRef}
          className="w-max p-1.5 max-h-[20vh] overflow-y-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"
        >
          {sections.map((s) => (
            <div
              key={s.id}
        ...
        >
          <AnimatePresence initial={false} mode="popLayout">
            {open ? (
              <motion.ul
                key="list"
                className="absolute inset-0 flex flex-col p-1.5 overflow-y-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden [mask-image:linear-gradient(to_bottom,transparent,black_5%,black_95%,transparent)]"
                initial={{
                  opacity: 0,
